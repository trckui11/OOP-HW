// 216303990 Arbel Feldman

/**
 * The pattern "NP {,} which is {{an example|a kind|a class} of} NP".
 *
 * @author Arbel Feldman arbel.feldman@live.biu.ac.il
 * @version JDK 19.0.2
 * @since 2022 -09-20
 */
public class NPwhichIsNP implements RegEx {
    public static final String REGEX =
            "(<np>([^<]+)</np>)( ,|) which is ((an example|a kind|a class) of |)<np>([^<]+)</np>";
    public static final int HYPERNYM_GROUP = 6;
    public static final int HYPONYM_GROUP = 1;

    /**
     * Constructor.
     */
    public NPwhichIsNP() {
    }

    @Override
    public String regEx() {
        return REGEX;
    }

    @Override
    public int hypernymGroup() {
        return HYPERNYM_GROUP;
    }

    @Override
    public int hyponymGroup() {
        return HYPONYM_GROUP;
    }
}
